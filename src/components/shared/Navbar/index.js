  ...
  return (
    ...
    <header className="bg-green-700 text-white p-4 flex justify-between items-center">
      ...
      <nav className="hidden md:flex space-x-6">
        {navItems.map((item) => (
          <Link key={item.href} href={item.href} className={`hover:underline ${pathname === item.href ? "font-bold underline" : ""}`}>
            {item.label}
          </Link>
        ))}
      </nav>
      ...
      {isMenuOpen && (
        <nav className="md:hidden bg-green-700">
          ...
          <div className="px-4 py-2 border-t border-green-600">
            ...
            <button className="block mt-2 hover:underline">Cerrar sesión</button>
          </div>
        </nav>
      )}
